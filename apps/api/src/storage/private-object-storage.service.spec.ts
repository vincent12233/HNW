...
describe('Private object storage', () => {
  ...
  beforeEach(async () => { directory = await mkdtemp(join(tmpdir(), 'kyc-storage-')); process.env.PRIVATE_OBJECT_ROOT = directory; });
  afterEach(async () => { if (previous === undefined) delete process.env.PRIVATE_OBJECT_ROOT; else process.env.PRIVATE_OBJECT_ROOT = previous; await rm(directory, { recursive: true, force: true }); });
  it('uses an absolute root without prepending the working directory', async () => {
    const storage = new PrivateObjectStorageService();
    const bytes = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a1ZkAAAAASUVORK5CYII=', 'base64');
  ...
  it('rejects traversal including siblings with the same root prefix', async () => {
    ...
    await expect(storage.get(directory + '-sibling/file')).rejects.toThrow('Invalid object key');
  });
});
