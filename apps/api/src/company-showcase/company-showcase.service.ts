...
export class CompanyShowcaseService {
  ...
  private data(body: any) {
    ...
    return { name, tagline, description, logoUrl: body?.logoUrl?.trim() || null, videoUrl: body?.videoUrl?.trim() || null, websiteUrl: body?.websiteUrl?.trim() || null, sector: body?.sector?.trim() || null, status: body?.status === 'INACTIVE' ? 'INACTIVE' : 'ACTIVE', sortOrder: Number.isFinite(Number(body?.sortOrder)) ? Number(body.sortOrder) : 0 };
  }
