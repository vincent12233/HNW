...
export class MarketNewsService {
  ...
  private get feedUrls() {
    const configured = (
      process.env.MARKET_NEWS_RSS_URLS ||
      process.env.MARKET_NEWS_RSS_URL ||
      ''
    )
      .split(',')
  ...
  async latest(limit = 8) {
    ...
    try {
      ...
      if (process.env.MARKET_NEWS_PROVIDER !== 'rss')
        requests.push(this.fetchGdelt());
  ...
  private async fetchGdelt(): Promise<MarketNewsItem[]> {
    const url = new URL(
      process.env.MARKET_NEWS_API_URL ||
        'https://api.gdeltproject.org/api/v2/doc/doc',
    );
    if (url.protocol !== 'https:') throw new Error('HTTPS news API required');
    url.search = new URLSearchParams({
      query:
        '(Nifty OR Sensex OR "Indian stock market" OR "Bombay Stock Exchange") sourcelang:english',
      mode: 'artlist',
      format: 'json',
      maxrecords: '20',
      timespan: '3d',
    ...
    return (Array.isArray(json.articles) ? json.articles : []).flatMap(
      (article) => {
        if (
          !article.title ||
          /[\u3400-\u9fff]/.test(article.title) ||
          !this.safeHttpUrl(article.url)
        )
          return [];
        const stamp = article.seendate?.replace(
        ...
        return [
          {
            ...
            url: article.url,
            imageUrl:
              article.socialimage && this.safeHttpsUrl(article.socialimage)
                ? article.socialimage
                : null,
  ...
  private parseItem(xml: string, index: number): MarketNewsItem | null {
    ...
    if (!title || /[\u3400-\u9fff]/.test(title) || !this.safeHttpUrl(link))
      return null;
    const source =
      this.text(xml, 'source') || new URL(link).hostname.replace(/^www\./, '');
    const imageCandidate =
      /<(?:media:content|enclosure)[^>]+url=["']([^"']+)["']/i.exec(xml)?.[1] ||
      /<img[^>]+src=["']([^"']+)["']/i.exec(
    ...
    return {
      id: `${new URL(link).hostname}-${Date.parse(publishedAt)}-${index}`,
      title,
      source,
      url: link,
      imageUrl:
        imageCandidate && this.safeHttpsUrl(imageCandidate)
          ? imageCandidate
          : null,
      publishedAt,
    };
