...
describe('Client profile and asset history', () => {
  it('updates membership and records previous and new tiers in one transaction', async () => {
    ...
    await expect(
      service.updateTier('admin', 'client', 'GOLD'),
    ).resolves.toEqual({ id: 'client', clientTier: 'GOLD' });
  ...
  it('rejects arbitrary avatar payloads and stores a resized raster image', async () => {
    ...
    await expect(
      service.updateAvatar('client', Buffer.from('<svg/>').toString('base64')),
    ).rejects.toThrow('valid JPEG');
    const png = await sharp({
      create: { width: 10, height: 20, channels: 3, background: '#ff0000' },
    })
      .png()
      .toBuffer();
    const result = await service.updateAvatar('client', png.toString('base64'));
    const metadata = await sharp(
      Buffer.from(result.avatarData!, 'base64'),
    ).metadata();
  ...
  it('rejects unsupported languages and themes without writing preferences', async () => {
    ...
    await expect(
      service.updatePreferences('client', { language: 'xx' }),
  ...
  it('updates only the name and never requires or changes email', async () => {
    ...
    const service = new ClientExperienceService({ user: { update } } as any);
    await service.updateProfile('client', {
      fullName: ' Asha Kumar ',
          ...
          positions: [
            {
              ...
              averagePrice: 10,
              realizedPnl: 5,
              instrument: { category: 'IT', quote: { lastPrice: 12 } },
            },
            {
              quantity: 3,
              averagePrice: 20,
              realizedPnl: 0,
              instrument: { category: 'IPO', quote: { lastPrice: 25 } },
            },
          ],
        }),
      },
  ...
  it('rejects unrecognized periods before database work', async () => {
    const service = new ClientExperienceService({} as any);
    await expect(service.assetHistory('client', 'constructor')).rejects.toThrow(
      'Unsupported period',
    );
  });
});
