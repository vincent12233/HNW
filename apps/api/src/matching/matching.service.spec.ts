...
describe('MatchingService', () => {
  ...
  it('settles a BUY fill using current account balances', async () => {
    ...
    const order = {
      ...
      instrument: { exchange: 'NSE', symbol: 'RELIANCE', quote: freshQuote(price) },
    };
    const tx = {
  ...
  it('keeps a DAY limit order open when the quote is stale', async () => {
    const price = new Prisma.Decimal('100');
    const order = {
      id: 'order-stale-day', accountId: 'account-1', instrumentId: 'instrument-1',
      side: 'BUY', type: 'LIMIT', timeInForce: 'DAY', status: 'OPEN',
      quantity: 2, filledQuantity: 0, limitPrice: new Prisma.Decimal('100'),
      averageFillPrice: null, frozenAmount: new Prisma.Decimal('200'),
      ...
      instrument: {
        ...
        quote: { askPrice: price, bidPrice: price, lastPrice: price, asOf: new Date(Date.now() - 180000) },
      },
    };
    const tx = {
      order: { findUnique: jest.fn().mockResolvedValueOnce(order).mockResolvedValueOnce(order), update: jest.fn() },
      trade: { create: jest.fn() },
    } as any;
    await createService(tx).matchOrder(order.id);
  ...
  it('cancels a non-DAY order when no fresh quote is available', async () => {
    const price = new Prisma.Decimal('100');
    const order = {
      id: 'order-stale-ioc', accountId: 'account-1', instrumentId: 'instrument-1',
    ...
    const tx = {
      ...
      account: { findUniqueOrThrow: jest.fn().mockResolvedValue({ cashBalance: new Prisma.Decimal('1000'), frozenBalance: new Prisma.Decimal('200') }), update: jest.fn() },
      accountTransaction: { create: jest.fn() },
      trade: { create: jest.fn() },
    } as any;
    await createService(tx).matchOrder(order.id);
  ...
  it('partially fills an IOC order then cancels and releases the remainder', async () => {
    ...
    const tx = {
      order: {
        findUnique: jest.fn().mockResolvedValueOnce(initialOrder).mockResolvedValueOnce(partiallyFilled).mockResolvedValueOnce({ id: initialOrder.id, status: 'CANCELLED' }),
        findUniqueOrThrow: jest.fn().mockResolvedValue(partiallyFilled), update: jest.fn(),
      },
      account: {
        findUniqueOrThrow: jest.fn().mockResolvedValueOnce({ cashBalance: new Prisma.Decimal('1000'), frozenBalance: new Prisma.Decimal('800') }).mockResolvedValueOnce({ cashBalance: new Prisma.Decimal('500'), frozenBalance: new Prisma.Decimal('300') }),
  ...
  it('returns price improvement from a partial BUY fill to buying power', async () => {
    ...
    const order = {
      id: 'order-price-improvement', accountId: 'account-1', instrumentId: 'instrument-1',
      side: 'BUY', type: 'LIMIT', timeInForce: 'DAY', status: 'OPEN',
      quantity: 8, filledQuantity: 0, limitPrice: new Prisma.Decimal('100'),
      averageFillPrice: null, frozenAmount: new Prisma.Decimal('800'),
      account: { cashBalance: new Prisma.Decimal('1000') },
    ...
    const tx = {
      order: { findUnique: jest.fn().mockResolvedValueOnce(order).mockResolvedValueOnce({ id: order.id, status: 'PARTIALLY_FILLED' }), update: jest.fn() },
      account: {
        findUniqueOrThrow: jest.fn().mockResolvedValue({ cashBalance: new Prisma.Decimal('1000'), frozenBalance: new Prisma.Decimal('800') }),
        update: jest.fn().mockResolvedValue({ cashBalance: new Prisma.Decimal('525') }),
      },
      accountTransaction: { create: jest.fn() },
      position: { findUnique: jest.fn().mockResolvedValue(null), upsert: jest.fn() },
      trade: { create: jest.fn() },
  ...
  it('weights order and position averages from actual prices on a later fill', async () => {
    ...
    const order = {
      id: 'order-later-fill', accountId: 'account-1', instrumentId: 'instrument-1',
      side: 'BUY', type: 'LIMIT', timeInForce: 'DAY', status: 'PARTIALLY_FILLED',
      quantity: 8, filledQuantity: 5, limitPrice: new Prisma.Decimal('100'),
      averageFillPrice: new Prisma.Decimal('95'), frozenAmount: new Prisma.Decimal('300'),
    ...
    const tx = {
      order: { findUnique: jest.fn().mockResolvedValueOnce(order).mockResolvedValueOnce({ id: order.id, status: 'FILLED' }), update: jest.fn() },
      account: {
        findUniqueOrThrow: jest.fn().mockResolvedValue({ cashBalance: new Prisma.Decimal('525'), frozenBalance: new Prisma.Decimal('300') }),
        update: jest.fn().mockResolvedValue({ cashBalance: new Prisma.Decimal('255') }),
      },
      accountTransaction: { create: jest.fn() },
      position: {
        findUnique: jest.fn().mockResolvedValue({ quantity: 5, averagePrice: new Prisma.Decimal('80') }),
        upsert: jest.fn(),
      },
      trade: { create: jest.fn() },
    } as any;
    ...
    expect(tx.position.upsert).toHaveBeenCalledWith(expect.objectContaining({
