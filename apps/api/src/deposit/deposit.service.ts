...
export class DepositService {
  ...
  async submitToFinanceBySupport(
    ...
    if (!conversationId)
      throw new BadRequestException('Support conversation is required');
    if (!/^(?!0+(?:\.0{1,2})?$)\d+(?:\.\d{1,2})?$/.test(amountText)) {
      throw new BadRequestException(
        'Enter a positive deposit amount with up to 2 decimals',
      );
    }
    ...
    const deposit = await this.prisma.$transaction(
      async (tx) => {
        ...
        if (conversation.assignedToId !== supportUserId) {
          throw new BadRequestException(
            'This conversation is not assigned to the current dedicated operator',
          );
        }
        ...
        const customer = await tx.user.findFirst({
          where: {
            ...
            role: 'CLIENT',
            assignedBusinessId: supportUserId,
        ...
        if (!customer) {
          throw new BadRequestException(
            'This customer is outside the dedicated operator scope',
          );
        }
        const account = await tx.account.findUnique({
          where: { userId: conversation.clientId },
        });
        if (!account) throw new NotFoundException('Customer account not found');
        const duplicate = await tx.depositRequest.findUnique({
          where: { referenceId },
        });
        if (duplicate)
          throw new BadRequestException(
            'This payment reference has already been confirmed',
          );
        const created = await tx.depositRequest.create({
          data: {
            ...
            note: [
              ...
              `Submitted by support ${supportUserId}`,
            ]
              .filter(Boolean)
              .join(' | ')
              .slice(0, 500),
  ...
  private async assertDepositVisible(
    userId: string,
    role: string,
    actorId: string,
