...
describe('OtcService review authorization', () => {
  it('prevents a business operator from rejecting another operator customer order', async () => {
    const tx = {
      otcOrder: {
        findUnique: jest.fn().mockResolvedValue({
          id: 'otc-1',
          status: 'PENDING',
          account: { userId: 'client-1', user: { assignedBusinessId: 'business-owner' } },
          instrument: { symbol: 'LMW' },
        }),
        updateMany: jest.fn(),
      },
      user: { findUnique: jest.fn().mockResolvedValue({ id: 'business-other', role: 'BUSINESS' }) },
    ...
    await expect(service.reject('business-other', 'otc-1', 'Rejected'))
