...
export class WithdrawalPinService {
  ...
  async change(
    userId: string,
    body: { currentPassword?: unknown; currentPin?: unknown; newPin?: unknown },
  ) {
    ...
    const result = await this.prisma.$transaction(
      async (tx) => {
        ...
        if (
          ...
          user.withdrawalPinLockedUntil > new Date()
        ) {
          return 'Too many attempts. Try again in 15 minutes';
        }
        if (!newPin && !user.withdrawalPinHash)
          return 'Set your withdrawal PIN in Profile first';
        const passwordValid =
          !newPin ||
        ...
        const pinValid =
          ...
          (typeof pin === 'string' &&
            /^\d{6}$/.test(pin) &&
            (await bcrypt.compare(pin, user.withdrawalPinHash)));
        if (!passwordValid || !pinValid) {
          const attempts = user.withdrawalPinLockedUntil
            ? 1
            : user.withdrawalPinAttempts + 1;
        ...
        await tx.user.update({
          ...
          data: {
            ...
            ...(newPin
              ? { withdrawalPinHash: await bcrypt.hash(newPin, 12) }
              : {}),
          },
        });
        return null;
