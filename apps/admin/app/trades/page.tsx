...
export default function TradesPage() {
  ...
  async function loadRecords() {
    ...
    try {
      const response = await api.get<TradesResponse>("/admin/trades", {
        params: {
          page: 1,
          pageSize: 100,
      ...
      setError(responseMessage || "成交数据加载失败",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AdminShell>
      <Space orientation="vertical" size="large" style={{ width: "100%" }}>
        <div>
          <Title level={2}>成交查询</Title>
          <Paragraph type="secondary">
            查看客户实际成交记录，支持按成交编号、订单号、客户、手机号和交易账号搜索。
          </Paragraph>
        </div>
        ...
        <Card>
          <Space wrap>
            <Input
              ...
              placeholder="搜索成交号、订单号、客户、手机号、交易账号"
              value={search}
