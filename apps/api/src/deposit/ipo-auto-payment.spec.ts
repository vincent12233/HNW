...
describe('IPO automatic payment from approved deposits', () => {
  function setup(amount: number) {
        ...
        findMany: jest.fn().mockResolvedValue([
          {
            ...
            ipoApplication: {
              ...
              allocatedPrice: 10,
              allocatedAmount: 100,
              ipo: { symbol: 'ABC', instrumentId: 'stock' },
            },
          },
        ]),
        update: jest.fn(),
    ...
    const settle = jest
      .spyOn(service as any, 'settleIpoApplication')
      .mockResolvedValue({});
    return { service, tx, settle };
  }

  it('removes the manual repayment route', () => {
    expect(Object.getOwnPropertyNames(ClientIpoController.prototype)).not.toContain(
      'payDebt',
    );
  });
