...
export default function AdminLoginPage() {
  ...
  async function submit(values: Values) {
    ...
    try {
      ...
      if (!["ADMIN", "MANAGER", "FINANCE", "BUSINESS"].includes(data.user?.role) || (deploymentRole && data.user?.role !== deploymentRole)) { setError(deploymentRole ? `此入口仅允许${backendRoleLabels[deploymentRole]}登录。` : "此账号没有后台访问权限。"); return; }
      localStorage.setItem("adminUser", JSON.stringify(data.user));
      const home: Record<string,string> = { ADMIN:"/dashboard", MANAGER:"/team", FINANCE:"/deposits", BUSINESS:"/business-customers" };
