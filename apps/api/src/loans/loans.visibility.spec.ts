...
describe('Loan record visibility', () => {
  ...
  it.each(['client-a', 'client-b'])(
    'limits client records to %s and excludes internal fields',
    async (userId) => {
  ...
  it.each(['business-a', 'business-b'])(
    'retains ownership for %s when searching',
    async (userId) => {
      const { service, findMany } = fixture();
      await service.list(userId, UserRole.BUSINESS, {
        search: ' outside-client ',
        status: 'PENDING',
      });
      ...
      expect(where.OR).toContainEqual({
        orderNo: { contains: 'outside-client', mode: 'insensitive' },
      });
    },
  );
