...
describe('LoansService concurrency protection', () => {
  it.each([0.01, 100.25, 123456.78])('credits exactly %s and rejects a second approval', async (amount) => {
    const loan = {
      id: 'loan-1', orderNo: 'LN1', status: 'PENDING', accountId: 'account-1',
      requestedAmount: new Prisma.Decimal(0), interestRate: new Prisma.Decimal(0),
      dueDate: null, note: null,
      account: { cashBalance: new Prisma.Decimal('200.15'), user: { usedInviteCode: null } },
    };
    const tx = {
      loanApplication: {
        findUnique: jest.fn().mockImplementation(async () => loan),
        updateMany: jest.fn().mockImplementation(async () => { loan.status = 'DISBURSED'; return { count: 1 }; }),
    ...
    await expect(service.approve(loan.id, 'finance', UserRole.FINANCE, { approvedAmount: amount }))
