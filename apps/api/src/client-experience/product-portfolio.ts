...
export function productCategory(
  value: string | null,
): (typeof productCategories)[number] | null {
...
export function summarizeProducts(positions: PositionInput[]) {
  ...
  for (const position of positions) {
    ...
    group.positions.push({
      ...
      currentValue,
      unrealizedPnl,
      valuationSource: hasQuote ? 'MARKET' : 'COST',
    });
  }
  const currentValue = groups.reduce(
  ...
  const unrealizedPnl = groups.reduce(
    (sum, group) => sum + group.unrealizedPnl,
    0,
  );
  const categories = groups.map((group) => ({
    ...group,
    allocationPercent:
      currentValue > 0 ? (group.currentValue / currentValue) * 100 : 0,
    totalPnl: group.realizedPnl + group.unrealizedPnl,
    unrealizedReturnPercent:
      group.invested > 0 ? (group.unrealizedPnl / group.invested) * 100 : null,
  }));
  const ranked = categories
    .filter((group) => group.invested > 0)
    .sort((a, b) => b.unrealizedReturnPercent! - a.unrealizedReturnPercent!);
  return {
    currentValue,
    invested,
    realizedPnl,
    ...
    positionCount: groups.reduce(
      (sum, group) => sum + group.positions.length,
      0,
    ),
    bestSegment: ranked[0]?.category ?? null,
    categories,
  };
}
