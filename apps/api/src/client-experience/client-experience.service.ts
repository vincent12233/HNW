...
export class ClientExperienceService {
  ...
  async changePassword(
    ...
    if (
      ...
      throw new BadRequestException(
        'Password must be 8 characters or more and at most 72 bytes',
      );
    if (
  ...
  async addBank(userId: string, body: any) {
    ...
    if (
      ...
      throw new BadRequestException('Enter valid bank account details');
    ...
    if (normalizeName(accountHolder) !== normalizeName(user.fullName)) {
      throw new BadRequestException(
        'Bank account holder must match the KYC account name',
      );
    }
    ...
    if (!approvedKyc.length)
      throw new BadRequestException(
        'Approved KYC is required before adding a bank account',
      );
    const duplicate = await this.prisma.bankAccount.findFirst({
      where: { userId, accountNumber, ifscCode },
    });
    if (duplicate)
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          body: `${bankName} is now available for withdrawals.`,
          referenceId: bank.id,
        },
      });
      return bank;
  ...
  async updatePreferences(userId: string, body: any) {
    ...
    const data = {
      ...
      ...(typeof body.orderNotifications === 'boolean'
        ...
        : {}),
      ...(typeof body.accountNotifications === 'boolean'
        ? { accountNotifications: body.accountNotifications }
        : {}),
      ...(typeof body.supportNotifications === 'boolean'
        ? { supportNotifications: body.supportNotifications }
        : {}),
  ...
  async reconciliation(userId: string) {
    ...
    for (const position of account.positions) {
      ...
      const value = moneyDecimal(
        new Prisma.Decimal(position.quantity).mul(price),
      );
      ...
      if (category === 'OTC') categories.OTC = categories.OTC.add(value);
        ...
        categories.INST = categories.INST.add(value);
    }
    ...
    await this.prisma.portfolioSnapshot.create({
      data: {
        ...
        cashValue: cash,
        instValue: moneyDecimal(categories.INST),
        otcValue: moneyDecimal(categories.OTC),
        ipoValue: moneyDecimal(categories.IPO),
  ...
  async updateAvatar(userId: string, value: unknown) {
    ...
    try {
      ...
      throw new BadRequestException('Choose a valid JPEG, PNG or WebP image');
    }
    return this.prisma.user.update({
      where: { id: userId },
      data: { avatarData },
  ...
  async assetHistory(userId: string, period: string) {
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        for (const position of account.positions) {
          const quote = moneyDecimal(position.instrument.quote?.lastPrice ?? 0);
          const price = quote.gt(0)
            ? quote
            : moneyDecimal(position.averagePrice);
          const marketValue = moneyDecimal(
            new Prisma.Decimal(position.quantity).mul(price),
          );
          ...
          const category = productCategory(position.instrument.category);
          if (category === 'IPO')
            categories.ipoValue = categories.ipoValue.add(marketValue);
          else if (category === 'OTC')
            categories.otcValue = categories.otcValue.add(marketValue);
          else if (category === 'Institutional')
            categories.instValue = categories.instValue.add(marketValue);
        }
        const latest = await tx.portfolioSnapshot.findFirst({
          where: { accountId: account.id, profitValue: { not: null } },
          orderBy: { capturedAt: 'desc' },
        });
        ...
        const productRows = rows.filter(
          (row) => row.productProfitValue != null,
        );
        return {
          period,
          from: rows[0]?.capturedAt ?? null,
          to: rows.at(-1)?.capturedAt ?? null,
          profitChange:
            rows.length < 2
              ? null
              : Number(
                  moneyDecimal(rows.at(-1)!.profitValue!).sub(
                    moneyDecimal(rows[0].profitValue!),
                  ),
                ),
          productProfitChange:
            productRows.length < 2
              ? null
              : Number(
                  moneyDecimal(productRows.at(-1)!.productProfitValue!).sub(
                    moneyDecimal(productRows[0].productProfitValue!),
                  ),
                ),
          ...
          points: sampled.map((row) => ({
            ...
            productValue: Number(
              moneyDecimal(row.instValue).add(row.otcValue).add(row.ipoValue),
            ),
          })),
        };
      },
      ...
      async (tx) => {
        ...
        const [orders, otc, ipos] = await Promise.all([
          tx.order.findMany({
            where: {
              accountId: account.id,
              instrument: { category: categoryFilter },
            },
            include: {
              instrument: { select: { symbol: true, category: true } },
            },
            orderBy: { updatedAt: 'desc' },
            take: 50,
          }),
          tx.otcOrder.findMany({
            where: { accountId: account.id },
            include: { instrument: { select: { symbol: true } } },
            orderBy: { updatedAt: 'desc' },
            take: 50,
          }),
          tx.ipoApplication.findMany({
            where: { accountId: account.id },
            include: { ipo: { select: { symbol: true } } },
            orderBy: { updatedAt: 'desc' },
            take: 50,
          }),
        ]);
        const activity = [
          ...orders.map((order) => ({
            ...
            type: 'ORDER',
            category: productCategory(order.instrument.category),
            ...
            amount:
              order.averageFillPrice == null
                ? null
                : Number(order.averageFillPrice) * order.filledQuantity,
            at: order.updatedAt,
          })),
          ...otc.map((order) => ({
            id: order.id,
            ...
            status: order.status,
            quantity: order.quantity,
            filledQuantity: order.status === 'APPROVED' ? order.quantity : 0,
            amount: Number(order.amount),
            at: order.updatedAt,
          })),
          ...ipos.map((application) => ({
            id: application.id,
            ...
            filledQuantity: application.allocatedQuantity ?? 0,
            amount:
              application.allocatedAmount == null
                ? null
                : Number(application.allocatedAmount),
        ...
        return {
          asOf: new Date(),
          ...summarizeProducts(account.positions),
          history,
          activity,
        };
      },
  ...
  adminBanks(role?: string) {
    ...
    return this.prisma.bankAccount.findMany({
      ...
      include: {
        user: {
          select: { id: true, fullName: true, phone: true, customerNo: true },
        },
      },
      orderBy: { createdAt: 'desc' },
      take: 500,
