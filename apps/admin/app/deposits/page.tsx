...
export default function DepositsPage() {
  ...
  const columns: ColumnsType<Deposit> = [
    ...
    { title: "状态", width: 120, render: () => <Tag color="orange">{dedicatedOperator ? "待运营审核" : "待财务上分"}</Tag> },
    { title: "客服备注", width: 240, render: (_, r) => r.note || "-" },
    { title: "操作", fixed: "right", width: 210, render: (_, r) => <Space><Button type="primary" icon={<CheckOutlined />} disabled={!!processingId || loading || !!loadError} loading={processingId === r.id} onClick={() => approve(r)}>确认到账并上分</Button><Button danger icon={<CloseOutlined />} disabled={!!processingId || loading || !!loadError} onClick={() => {setRejectNote(""); setRejecting(r);}}>拒绝</Button></Space> },
  ];

  return <AdminShell>
    <Space orientation="vertical" size="large" style={{ width: "100%" }}>
      {loadError && <Alert type="error" showIcon title={loadError} action={<Button onClick={load} loading={loading}>重试</Button>} />}
      <div><Title level={2}>客户存款核对与上分</Title><Paragraph type="secondary">{dedicatedOperator ? "仅显示归属当前专用运营员、使用固定邀请码注册的客户。请核对付款流水和实际到账金额后处理。" : "财务须自行核对收款账户、付款流水号和实际到账金额，确认后直接上分。固定邀请码客户不会出现在这里。"}</Paragraph></div>
