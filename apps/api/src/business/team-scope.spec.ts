...
describe('Team editing scope', () => {
  it('requires an owned business account for manager edits', async () => {
    ...
    await expect(service.business('manager', 'other-business')).rejects.toThrow(
      'not found in your team',
    );
    expect(prisma.user.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          id: 'other-business',
          role: 'BUSINESS',
          businessCreatorId: 'manager',
          deletedAt: null,
        },
      }),
    );
    prisma.user.findFirst.mockResolvedValue({ id: 'own-business' } as never);
    await expect(service.business('manager', 'own-business')).resolves.toBe(
      'own-business',
    );
  });
  it.each(['BUSINESS', 'FINANCE', 'SUPPORT', 'CLIENT'])(
    'rejects %s from team management',
    async (role) => {
